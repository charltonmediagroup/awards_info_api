...
export async function POST(req: Request) {
  try {
    ...
    if (!name) return NextResponse.json({ error: "Region name required" }, { status: 400 })

    const filePath = path.join(awardsDir, `${name}.json`)
    if (fs.existsSync(filePath)) return NextResponse.json({ error: "Region already exists" }, { status: 400 })
